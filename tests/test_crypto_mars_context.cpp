...
static Bytes make_key(size_t bits) {
    ...
    for (size_t i = 0; i < key.size(); ++i) {
        key[i] = static_cast<uint8_t>(i * 0x17 + 0x0B);
    }
    return key;
}
...
static Bytes make_iv(size_t block_bytes, SymmetricEncryptionMode mode) {
    ...
    for (size_t i = 0; i < iv.size(); ++i) {
        iv[i] = static_cast<uint8_t>(i * 0x5D + 0x27);
    }
    return iv;
}
...
TEST(MARSContext, AllModesAllPaddingsRoundtrip128) {
...
TEST(MARSContext, AllModesAllPaddingsRoundtrip256) {
    for (SymmetricEncryptionMode mode : {SymmetricEncryptionMode::ECB,
                                         SymmetricEncryptionMode::CBC,
                                         SymmetricEncryptionMode::PCBC,
                                         SymmetricEncryptionMode::CFB,
...
TEST(MARSContext, ParallelEncryptMatchesSequential) {
...
TEST(MARSContext, ParallelDecryptMatchesSequential) {
    for (SymmetricEncryptionMode mode : {SymmetricEncryptionMode::ECB,
                                         SymmetricEncryptionMode::CTR,
                                         SymmetricEncryptionMode::CBC}) {
...
TEST(MARSContext, FileEncryptionAsyncRoundtrip) {
    const std::string src = "mars_file_test.bin";
    const std::string enc = "mars_file_test.enc";
    const std::string dec = "mars_file_test.out";
...
TEST(MARSContext, DifferentIvGivesDifferentCiphertext) {
